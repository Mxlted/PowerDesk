...
- **StartupPilot** — Inspect and toggle everything Windows runs at sign-in: HKCU/HKLM `Run` and `RunOnce`, Startup folders (per-user and all-users), scheduled tasks with login/boot triggers, and services. Startup entries use Task Manager-style enabled/disabled state where possible; services have a separate view for Automatic, Manual, and Disabled startup types. Tracks every change with an undo-able history and CSV export.
...
```
dotnet publish PowerDesk.csproj -c Release -r win-x64 --self-contained -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -o "../publish"
```
...
- Changing all-users Startup folder approval state
- Changing services between Automatic, Manual, and Disabled
